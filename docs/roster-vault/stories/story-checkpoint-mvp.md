...
IN PROGRESS — Task 0 and Task 2 complete. Task 1 IN PROGRESS (repo created and pushed; Nigel is connecting Amplify Hosting himself). Task 3 IN PROGRESS (Flutter scaffold + device keypair generation implemented via `biometric_signature`; real-device/emulator verification still outstanding).
...
- **Status:** IN PROGRESS — repo created and pushed; Amplify Hosting connection is Nigel's next step
...
- **Work done:** Nigel created a private GitHub repo, `https://github.com/nrbondnz/roster-vault`. Initial commit pushed to `main` (119 files — docs, agents, Amplify backend, Flutter scaffold). **Remaining:** connect that repo as the git provider for Amplify Gen 2 Hosting in the AWS Amplify Console, so a push triggers a hosted backend build — not just a locally-run `ampx sandbox`. Nigel is doing this step himself.
- **Verify:** A push to `main` triggers a build in the Amplify Console for this app, and it completes successfully.
- **Save:** Repo URL recorded here and in [[../Build and Deploy/Build and Deploy Index|Build and Deploy Index]]. Amplify Hosting app ID to be recorded once Nigel creates it.
- **Depends on:** Task 0
...
- **Verify so far:** `flutter analyze` and `flutter test` both clean/passing. `flutter build windows --debug` succeeds. **Not yet verified:** actually running the app and confirming (a) a fresh install generates the key/deviceId exactly once, (b) relaunching doesn't regenerate them, (c) the public key is genuinely ECDSA P-256. **Important caveat discovered:** the plugin's own docs state Windows ignores the requested signature type and always issues RSA-2048 instead — so a Windows desktop run can prove the UI/storage wiring but *cannot* verify the actual P-256 guarantee. That verification needs an Android or iOS run. No Android emulator (AVD) exists on this machine yet (`flutter emulators` reports none) — creating one involves a sizeable system-image download, so held off pending Nigel's steer rather than done unattended.
